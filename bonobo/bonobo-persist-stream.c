	   ...
	   CORBA_Environment *ev)
{
	...
	if (ps->load_fn != NULL)
		...
		result = (*stream->parent_class.load)(ps, stream);
	   ...
	   CORBA_Environment *ev)
{
	...
	if (ps->save_fn != NULL)
		...
		result = (*stream->parent_class.save)(ps, stream);
...
impl_get_size_max (PortableServer_Servant servant, CORBA_Environment * ev)
{
	...
	return ((*stream->parent_class.get_size_max)(ps));
}
...
gnome_persist_stream_class_init (GnomePersistStreamClass *class)
{
	 ...
	 */
	object_class->destroy = gnome_persist_stream_destroy;
				...
				void *closure)
{
	...
	gnome_persist_construct (GNOME_PERSIST (ps));
