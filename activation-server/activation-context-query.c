...
qexp_free(QueryExpr *qexp)
{
  ...
  switch (qexp->type)
    {
    case EXPR_FUNCTION:
      g_slist_foreach(qexp->u.function_value.arguments, (GFunc)qexp_free, NULL);
      ...
      if (qexp->u.constant_value.value_known)
	{
	  switch (qexp->u.constant_value.type)
	    ...
	    case CONST_STRING:
	      ...
	      break;
	    case CONST_STRINGV:
	      g_strfreev(qexp->u.constant_value.u.v_stringv);
	      break;
	    default:
	      break;
	    }
...
qexp_function_new(char *name, GSList *exprlist)
{
  ...
  for (cur = exprlist; cur && !((QueryExpr *)cur->data)->has_fields; cur = cur->next) /* */;
...
qexp_dump(QueryExpr *exp)
{
  switch (exp->type)
	...
	for (cur = exp->u.function_value.arguments; cur; cur = cur->next)
	  {
	    qexp_dump(cur->data);
	    if (cur->next)
	      g_print(", ");
...
qexp_constant_bool(const QueryExprConst *c)
{
  if (c->value_known)
    switch (c->type)
      {
      case CONST_BOOLEAN: return c->u.v_boolean;
      case CONST_NUMBER: return (c->u.v_number != 0.0);
      case CONST_STRING: return (c->u.v_string != NULL);
      case CONST_STRINGV: return (c->u.v_stringv != NULL);
      }
...
qexp_constant_compare(const QueryExprConst *c1, const QueryExprConst *c2)
{
  if (c1->value_known && c2->value_known)
	...
	break;
      case CONST_NUMBER:
	{
	  gdouble diff = c2->u.v_number - c1->u.v_number;
...
typedef QueryExprConst (*QueryExprEvalFunc)(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx);
...
qexp_evaluate_function(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx)
{
...
qexp_func_has_one(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx)
{
  ...
  v2 = qexp_evaluate(si, e->u.function_value.arguments->next->data, qctx);
  ...
  else if (v1.type != CONST_STRINGV
	   || v2.type != CONST_STRINGV)
    {
  ...
  for (i = 0; i < qctx->nservers; i++)
    {
      ...
      new_val = qexp_evaluate(qctx->sil[i], e->u.function_value.arguments->data, qctx);
...
static QueryExprConst
qexp_evaluate_variable(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx)
{
  QueryExprConst retval;
  ...
  if (qctx->cctx)
    {
      ...
      CORBA_Context_get_values(qctx->cctx, NULL, 0, e->u.var_value, &nvout, &ev);
	...
	if (nvout->list->len > 0)
	  {
	    ...
	    retval.u.v_string = g_strdup(*(char **)nv->argument._value);
  ...
  if (si)
      ...
      else
	{
	  ...
	  if (i < si->attrs._length)
		...
		case OAF_A_STRING:
		  retval.type = CONST_STRING;
		  retval.u.v_string = av->_u.value_string;
		  break;
		case OAF_A_NUMBER:
		  retval.type = CONST_NUMBER;
		  retval.u.v_number = av->_u.value_number;
		  break;
		case OAF_A_BOOLEAN:
		  retval.type = CONST_BOOLEAN;
		  retval.u.v_boolean = av->_u.value_boolean;
		  break;
		case OAF_A_STRINGV:
		  {
...
qexp_evaluate_binop(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx)
{
  ...
  switch (e->u.binop_value.type)
    ...
    case OP_OR:
      retval.type = CONST_BOOLEAN;
      retval.u.v_boolean = qexp_constant_bool(&v1) || qexp_constant_bool(&v2);
      break;
    case OP_AND:
      retval.type = CONST_BOOLEAN;
      retval.u.v_boolean = qexp_constant_bool(&v1) && qexp_constant_bool(&v2);
...
/********** constants ************/
static QueryExprConst
qexp_evaluate_constant(OAF_ServerInfo *si, QueryExpr *e, QueryContext *qctx)
{
